# Permission catalogue (generated)

Generated by `make permissions` from each module's `registry.py`, and
checked in CI. Every code below is one a view or service actually
guards on, and every designation column is one the IAM can grant.

For the intended taxonomy of modules not yet built, see
[permission-catalog.md](permission-catalog.md).

## `directory`

| Code | What it allows | Held by |
|---|---|---|
| `directory.user.search` | Search the institute directory | `Dean Academic`, `acadadmin`, `placement_chairman`, `placement_coordinator`, `placement_officer` |

## `leave`

| Code | What it allows | Held by |
|---|---|---|
| `leave.request.create` | Apply for leave | `Assistant Professor`, `Associate Professor`, `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Junior Assistant`, `Professor`, `Registrar`, `Senior Assistant`, `Upper Division Clerk`, `acadadmin`, `dracad` |
| `leave.request.view_self` | See your own leave | `Assistant Professor`, `Associate Professor`, `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Junior Assistant`, `Professor`, `Registrar`, `Senior Assistant`, `Upper Division Clerk`, `acadadmin`, `dracad` |
| `leave.request.withdraw` | Withdraw your pending request | `Assistant Professor`, `Associate Professor`, `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Junior Assistant`, `Professor`, `Registrar`, `Senior Assistant`, `Upper Division Clerk`, `acadadmin`, `dracad` |
| `leave.substitute.respond` | Accept or decline standing in for someone | `Assistant Professor`, `Associate Professor`, `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Junior Assistant`, `Professor`, `Registrar`, `Senior Assistant`, `Upper Division Clerk`, `acadadmin`, `dracad` |
| `leave.request.review` | Review as unit head and record a recommendation | `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Professor`, `Registrar` |
| `leave.request.route` | Route a recommended request to the competent authority | `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `Registrar`, `acadadmin`, `dracad` |
| `leave.request.sanction` | Take the final decision on a leave request | `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Director`, `Registrar` |
| `leave.resumption.verify` | Verify a reported resumption of duty | `Deputy Registrar`, `acadadmin`, `dracad` |
| `leave.balance.view` | See leave balances across employees | `Dean (P&D)`, `Dean (R&D)`, `Dean Academic`, `Dean_s`, `Deputy Registrar`, `Director`, `HOD (CSE)`, `HOD (Design)`, `HOD (ECE)`, `HOD (Liberal Arts)`, `HOD (ME)`, `HOD (NS)`, `Professor`, `Registrar`, `acadadmin`, `dracad` |
| `leave.policy.manage` | Maintain leave policy and entitlement | `acadadmin` |
| `leave.calendar.manage` | Maintain the holiday and restricted-holiday calendar | `acadadmin` |
| `leave.offline.record` | Record leave sanctioned outside the system | `acadadmin` |

## `placement_cell`

| Code | What it allows | Held by |
|---|---|---|
| `placement_cell.job_posting.view` | See job postings | `Dean Academic`, `acadadmin`, `placement_chairman`, `placement_coordinator`, `placement_officer`, `student` |
| `placement_cell.job_posting.manage` | Create and publish postings | `placement_officer` |
| `placement_cell.application.view` | See all applications | `Dean Academic`, `acadadmin`, `placement_chairman`, `placement_coordinator`, `placement_officer` |
| `placement_cell.application.view_self` | See one's own applications | `student` |
| `placement_cell.application.create` | Apply to a posting | `student` |
| `placement_cell.application.review` | Shortlist and reject applications | `acadadmin`, `placement_coordinator`, `placement_officer` |
| `placement_cell.application.delete` | Withdraw an application | `student` |
| `placement_cell.application.auto_withdraw` | System auto-withdrawal | _service only_ |
| `placement_cell.interview.schedule` | Schedule interview rounds | `placement_officer` |
| `placement_cell.offer.issue` | Extend an offer | `placement_officer` |
| `placement_cell.offer.respond` | Accept or decline an offer | `student` |
| `placement_cell.offer.revoke` | Revoke an issued offer | `placement_officer` |
| `placement_cell.offer.expire` | System offer expiry | _service only_ |
| `placement_cell.company.manage` | Register and approve companies | `placement_officer` |
| `placement_cell.announcement.publish` | Publish announcements | `Dean Academic`, `placement_chairman`, `placement_officer` |
| `placement_cell.report.view` | See operational reports | `Dean Academic`, `acadadmin`, `placement_chairman`, `placement_coordinator`, `placement_officer` |
| `placement_cell.registration.debar` | Record conduct incidents and impose placement sanctions | `placement_officer` |
| `placement_cell.registration.manage` | Approve late registrations and re-registrations | `placement_officer` |
| `placement_cell.registration.self` | Register yourself for a placement season | `student` |
| `placement_cell.record.manage` | Record off-campus placements and chase offer letters | `placement_officer` |
| `placement_cell.academic_directory.view` | Browse every student's declared CPI | `Dean Academic`, `acadadmin`, `placement_chairman`, `placement_coordinator`, `placement_officer` |
