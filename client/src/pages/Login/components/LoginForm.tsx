...
import {
  Box, Button, Group, PasswordInput, Stack, Text, TextInput, Title,
  UnstyledButton, rem,
} from "@mantine/core";
...
const RequiredLabel = ({ children }: { children: React.ReactNode }) => (
  <Group gap={4} wrap="nowrap">
    <Text size="xs" fw={800} c="gray.6">{children}</Text>
    <Text size="xs" c="red" fw={700} style={{ lineHeight: 1 }}>*</Text>
  </Group>
);

const underlineStyle = (focused: boolean) => ({
...
/**
 * The credentials form. Purely presentational plus local field state; it
 * delegates the actual authentication to `onSubmit`.
 *
 * Ported from the operator console so the two doors look identical. The only
 * variable parts are the heading, the field label and the placeholder — a
 * recruiter signs in with an email, an institute user with a roll number.
 */
export default function LoginForm({
  isMobile, loading, error, onClearError, onSubmit, onBack,
  heading = "Login",
  identifierLabel = "USERNAME",
  identifierPlaceholder = "Username / Roll No.",
  identifierType = "text",
  footer,
}: {
  isMobile: boolean;
  ...
  const clearOnEdit = () => { if (error) onClearError?.(); };
  ...
  return (
    <Stack gap={isMobile ? 18 : 30} style={{ paddingTop: 20, paddingBottom: 40 }}>
      <Box>
        ...
        <Box style={{ position: "relative" }}>
          ...
          <Box
            ...
            style={{
              position: "absolute",
              bottom: -5, left: 0, width: 60, height: 3,
      ...
      {error && (
        <Box
          role="alert" aria-live="assertive" p="sm"
          style={{
        ...
        >
          ...
          <Text size="sm" fw={600} style={{ lineHeight: 1.35 }}>{error}</Text>
        </Box>
      )}
      ...
      <form onSubmit={handleSubmit} noValidate>
        <Stack gap={isMobile ? "md" : "lg"}>
          <TextInput
            label={<RequiredLabel>{identifierLabel}</RequiredLabel>}
            placeholder={identifierPlaceholder}
            type={identifierType}
            variant="unstyled" size="lg" radius={0}
            value={username}
            onChange={(e) => { setUsername(e.currentTarget.value); clearOnEdit(); }}
          ...
          <PasswordInput
            ...
            variant="unstyled" size="lg" radius={0}
            value={password}
            onChange={(e) => { setPassword(e.currentTarget.value); clearOnEdit(); }}
          ...
          <Button
            fullWidth size="xl" radius={0} type="submit"
            loading={loading} disabled={!isValid} aria-label="Sign in"
            style={{
              height: rem(64),
              background: !isValid
